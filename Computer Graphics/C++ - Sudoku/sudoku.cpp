...
void main() {
	...
	for (int i = 0; i < 9; i++) {
		int inx = getmaxx() / 2 - (lbox * 9 / 2) + (lbox / 2), iny = getmaxy()
				/ 2 - (bbox * 9 / 2) + (bbox / 2);
		for (int j = 0; j < 9; j++) {
			box[i][j].l = inx;
			inx += lbox;
	...
	while (ch != 27) {
		...
		if (button == 1) {
			...
			for (i = 0; i < 9; i++) {
				for (int j = 0; j < 9; j++) {
					if (x > box[i][j].l - (lbox / 2)
							&& y > box[i][j].b - (bbox / 2)
							&& x < box[i][j].l + (lbox / 2)
							&& y < box[i][j].b + (bbox / 2)) { //to check in which block the mouse has been clicked
						ch = getch();
		...
		if (ch == 13) {
			...
			if ((sum[0][0] + sum[0][1] + sum[0][2] + sum[1][0] + sum[1][1]
					+ sum[1][2] + sum[2][0] + sum[2][1] + sum[2][2]) == 45) {
				right++;
			}
			if ((sum[3][0] + sum[3][1] + sum[3][2] + sum[4][0] + sum[4][1]
					+ sum[4][2] + sum[5][0] + sum[5][1] + sum[5][2]) == 45) {
				right++;
			}
			if ((sum[6][0] + sum[6][1] + sum[6][2] + sum[7][0] + sum[7][1]
					+ sum[7][2] + sum[8][0] + sum[8][1] + sum[8][2]) == 45) {
				right++;
			}
			if (right == 27) {
				cleardevice();

...
void passworda() {
	...
	hidemouseptr();
	int i = 0, left = getmaxx() / 2 - 5, top = getmaxy() / 2, right = getmaxx()
			/ 2 + 5, bot = getmaxy() / 2;
	for (i = 0; i <= 140; i++) {
		setcolor(0);
		rectangle(left + 1, top + 1, right - 1, bot - 1);
	...
	while (1) {
		while (k != 1) {
			getmousepos(&button, &x, &y);
			if (button == 1) {
				if (x > getmaxx() / 2 - 20 && x < getmaxx() / 2 + 20
						&& y > bot - 50 && y < bot - 12)
					contactme();
				if (x > left + 10 && x < left + 50 && y > bot - 50
						&& y < bot - 12)
					exit(0);
				if (x > right - 50 && x < right - 10 && y > bot - 50
						&& y < bot - 12)
					front();
				if (x > left + 80 && y > top + 100 && x < right - 40
						&& y < top + 125)
					k = 1;
			}
		}
...
void passwordg() {
	hidemouseptr();
	int i, left = getmaxx() / 2 - 5, top = getmaxy() / 2, right = getmaxx() / 2
			+ 5, bot = getmaxy() / 2;
	...
	while (1) {
		...
		if (button == 1) {
			if (x > 5 && x < 50 && y > getmaxy() - 50 && y < getmaxy() - 5)
				frontm2();
			if (x > getmaxx() - 50 && x < getmaxx() - 5 && y > getmaxy() - 50
					&& y < getmaxy() - 5)
				front();
...
//master reset
	...
	rectangle((xi + 2 * d1 + 2 * d + xi + 3 * d1 + 2 * d) / 2 + 2,
			(ym + yi + 2 * d2 + 2 * d) / 2 - 2,
			((xi + 2 * d1 + 2 * d + xi + 3 * d1 + 2 * d) / 2 + 2 + xi + 3 * d1
					+ 2 * d - 20) / 2,
			((ym + yi + 2 * d2 + 2 * d) / 2 - 2 + yi + 2 * d2 + 2 * d + 15)
					/ 2);
	rectangle(
			(xi + 2 * d1 + 2 * d + 20
					+ (xi + 2 * d1 + 2 * d + xi + 3 * d1 + 2 * d) / 2 - 2) / 2,
			(ym + yi + 2 * d2 + 2 * d) / 2 + 2,
			(xi + 2 * d1 + 2 * d + xi + 3 * d1 + 2 * d) / 2 - 2,
...
//selfdestruct
	...
	while (1) {
		...
		if (x > xi + d1 + d && x < xi + 2 * d1 + d && y > yi && y < yi + d2)
			chh = 2;
		if (x > xi + 2 * d1 + 2 * d && x < xi + 3 * d1 + 2 * d && y > yi
				&& y < yi + d2)
			chh = 3;
		...
		if (x > xi && x < xi + d1 && y > yi + d2 + d && y < yi + 2 * d2 + d)
			chh = 5;
		if (x > xi + d1 + d && x < x + 2 * d1 + d && y > yi + d2 + d
				&& y < yi + 2 * d2 + d)
			chh = 6;
		if (x > xi + 2 * d1 + 2 * d && x < x + 3 * d1 + 2 * d && y > yi + d2 + d
				&& y < yi + 2 * d2 + d)
			chh = 7;
		if (x > xi + 3 * d1 + 3 * d && x < xm && y > yi + d2 + d
				&& y < yi + 2 * d2 + d)
			chh = 8;

		if (x > xi && x < xi + d1 && y > yi + 2 * d2 + 2 * d && y < ym)
			chh = 9;
		if (x > xi + d1 + d && x < x + 2 * d1 + d && y > yi + 2 * d2 + 2 * d
				&& y < ym)
			chh = 10;
		if (x > xi + 2 * d1 + 2 * d && x < xi + 3 * d1 + 2 * d
				&& y > yi + 2 * d2 + 2 * d && y < ym)
			chh = 11;
		if (x > xi + 3 * d1 + 3 * d && x < xm && y > yi + 2 * d2 + 2 * d
				&& y < ym)
			chh = 12;
...
//rectangle();
		...
		if (button == 1) {
			...
			if (x > xi + d1 + d && x < x + 2 * d1 + d && y > yi + d2 + d
					&& y < yi + 2 * d2 + d)
				exit(0);
			if (x > xi + 2 * d1 + 2 * d && x < x + 3 * d1 + 2 * d
					&& y > yi + d2 + d && y < yi + 2 * d2 + d)
				exit(0);
			if (x > xi + 3 * d1 + 3 * d && x < xm && y > yi + d2 + d
					&& y < yi + 2 * d2 + d)
				exit(0);
...
void masterreset() {
	int left = getmaxx() / 2 - 50, top = getmaxy() / 2, right = getmaxx() / 2
			+ 50, bot = getmaxy() / 2;
	...
	while (1) {
		getmousepos(&button, &x, &y);
		if (button == 1) {
			if (x > x1 + 10 && y > y1 + 2 * dd2 && x < x1 + dd1 - 5
					&& y < y2 - 10) {
				ofstream out;
			...
			if (x > x1 + dd1 + 5 && y > y1 + 2 * dd2 && x < x2 - 10
					&& y < y2 - 10)
				frontm1;
		}
	}
...
// <-
	...
	while (!kbhit() || button != 1) {
		...
		if (alhr == t.ti_hour) {
			if (almin == t.ti_min) {
				int left = getmaxx() / 2 - 50, top = getmaxy() / 2, right =
						getmaxx() / 2 + 50, bot = getmaxy() / 2;
				for (int k = 0; k <= 80; k++) {
					setcolor(0);
					rectangle(left + 1, top + 1, right - 1, bot - 1);
					setcolor(1);
				...
				while (1) {
					getmousepos(&button, &x, &y);
					if (button == 1) {
						if (x > x1 + 10 && y > y1 + 3 * dd2 + 5 && x < x2 - 10
								&& y < y2 - 10)
							gclock();
						if (x > x1 + 10 && y > y1 + dd2 + 5 && x < x1 + dd1 - 5
								&& y < y1 + 3 * dd2 - 5) {
						...
						if (x > x1 + dd1 + 5 && y > y1 + dd2 + 5
								&& x > x1 + 2 * dd1 - 5 && y < y1 + 3 * dd2 - 5)
							actalarm();
						if (x > x1 + 2 * dd1 + 5 && y > y1 + dd2 + 5
								&& x < x2 - 10 && y < y1 + 3 * dd2 - 5) {
							ofstream fout;
							fout.open("alarm.txt", ios::out);
							fout << "0000";
							fout.close();
...
void selfd() {
	int left = getmaxx() / 2 - 40, top = getmaxy() / 2, right = getmaxx() / 2
			+ 40, bot = getmaxy() / 2;
	for (int k = 0; k <= 80; k++) {
		setcolor(0);
		rectangle(left + 1, top + 1, right - 1, bot - 1);
